...
async function loginUser(data) {
  try {
    let res = await AxiosInstance.post("/users/login", data);
...
async function handleSubmit() {
  errors.value = [];
  if (!loginObj.value.email) {
    errors.value.push("Entering a email is required");
  } else {
    if (!/[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,3}$/.test(loginObj.value.email)) {
      errors.value.push("Email must be valid");
    }
  }
  if (!loginObj.value.password) {
